...
# set container docker groupid to host groupid so docker group membership works properly
if [ ! -z $HOST_DOCKER_GID ]
   then
      groupmod -g $HOST_DOCKER_GID docker
    else
      echo "##### no HOST_DOCKER_GID found - please set to host docker groupid before running this container"
fi
